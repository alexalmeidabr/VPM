...
if (!isBrowserRuntime) {
  ...
  const ui = {
    ...
    projectsBoardContainer: document.getElementById('projects-board-container'),
    projectListViewBtn: document.getElementById('project-list-view-btn'),
    projectBoardViewBtn: document.getElementById('project-board-view-btn'),
  ...
  let projectListViewMode = localStorage.getItem('projectListViewMode') === 'board' ? 'board' : 'list';
  ...
  const projectStatusOrder = () => Array.from(fields.projectStatus?.options || []).map((option) => String(option.value || option.textContent || '').trim()).filter(Boolean);
  const projectStatusForBoard = (project) => String(project?.projectStatus || '').trim() || 'No Status';
  const projectTypeForBoard = (project) => String(project?.projectType || '').trim() || 'No Type';
  const projectGoLiveDate = (project) => {
    const milestones = Array.isArray(project?.projectMilestones) ? project.projectMilestones : [];
    const goLive = milestones.find((milestone) => String(milestone?.name || '').trim().toLowerCase().replace(/[\s-]+/g, '') === 'golive'
      || String(milestone?.name || '').trim().toLowerCase().replace(/[\s-]+/g, '') === 'golivedate');
    return goLive ? (goLive.endDate || goLive.startDate || '') : '';
  };
  const orderedProjectStatuses = () => {
    const knownOrder = projectStatusOrder();
    const statuses = [...new Set(projects.map(projectStatusForBoard))];
    return [
      ...knownOrder.filter((status) => statuses.includes(status)),
      ...statuses.filter((status) => !knownOrder.includes(status)).sort((a, b) => a.localeCompare(b))
    ];
  };
  const orderedProjectTypes = () => {
    const configuredTypes = projectTypes.map((type) => String(type.name || '').trim()).filter(Boolean);
    const types = [...new Set(projects.map(projectTypeForBoard))];
    return [
      ...configuredTypes.filter((type) => types.includes(type)),
    ...
    types.forEach((type) => {
      ...
      statuses.forEach((status) => {
        ...
        columnProjects.forEach((project) => {
          ...
          card.type = 'button';
          card.className = 'projects-board-card';
          card.dataset.action = 'edit-project';
  ...
  const ensureAllocationStateShape = () => {
    ...
    allocationState.projects.forEach((project) => {
      project.vacancies = Array.isArray(project.vacancies) ? project.vacancies.map((vacancy) => normalizeAllocationVacancy(vacancy)) : [];
    });
  };
  ...
  const openAllocationVacancyModal = (projectId) => {
    ...
    if (ui.allocationVacancyAllocationInput) ui.allocationVacancyAllocationInput.value = '100';
    updateTextFields();
    modals.allocationVacancy?.open();
  };
  const addAllocationVacancyToProject = (projectId, areaId, allocation) => {
    ...
    if (!Number.isFinite(allocationValue) || allocationValue <= 0 || allocationValue > 100) return false;
    project.vacancies = [...(project.vacancies || []), normalizeAllocationVacancy({
  ...
  const renderAllocationWorkspace = () => {
    ...
    allocationState.projects.forEach((project) => {
      ...
      (project.vacancies || []).forEach((vacancy) => {
        const normalizedVacancy = normalizeAllocationVacancy(vacancy);
  ...
  const handleProjectAction = async (event) => {
    const button = event.target.closest('button[data-action]');
    if (!button) return;
  ...
  ui.projectsBody.addEventListener('click', handleProjectAction);
  ui.projectsBoardContainer?.addEventListener('click', handleProjectAction);
  ui.projectListViewBtn?.addEventListener('click', () => {
    projectListViewMode = 'list';
    localStorage.setItem('projectListViewMode', projectListViewMode);
    renderProjects();
  });
  ui.projectBoardViewBtn?.addEventListener('click', () => {
    projectListViewMode = 'board';
    localStorage.setItem('projectListViewMode', projectListViewMode);
    renderProjects();
  });
  ...
  ui.allocationVacancySaveBtn?.addEventListener('click', () => {
    ...
    const allocationValueRaw = String(ui.allocationVacancyAllocationInput?.value || '').trim();
    const allocationValue = Number(allocationValueRaw);
    if (!allocationValueRaw || !Number.isFinite(allocationValue)) {
    ...
    const added = addAllocationVacancyToProject(allocationVacancyTargetProjectId, selectedAreaId, allocationValue);
    if (!added) {
